...
// Capturamos el correo guardado desde la pantalla anterior
onMounted(() => {
...
function ocultarCorreo(email: string): string {
  ...
  const mitad = Math.max(Math.floor(usuario.length / 2), 1);
  return usuario.slice(0, 1) + "*".repeat(mitad) + "@" + dominio;
}
...
function reenviarEnlace() {
  ...
  // Aquí puedes agregar tu llamada a la API real para reenviar
}
</script>
